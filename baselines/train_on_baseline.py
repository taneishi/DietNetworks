...
def main(embedding_path, n_classes, which_method='pca', enc='triangle'):
    ...
    embedding_methods = [emb for emb in embedding_methods if '.npz' in emb and which_method in emb]

    if which_method == 'kmeans':
        embedding_methods = [emb for emb in embedding_methods if enc in emb]
